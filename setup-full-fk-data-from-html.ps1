# =====================================================
# setup-full-fk-data-from-html.ps1
# نسخه نهایی: Migrationها با FK و داده واقعی از final.html
# =====================================================
...
# ----------------------------
# Model ها + Migration با فیلدهای واقعی و FK
# ----------------------------
$modelsWithSchema = @(
...
foreach ($m in $modelsWithSchema) {
    ...
    # ویرایش migration برای FK و type
...
# ----------------------------
# Seed کردن داده‌های واقعی از final.html
# ----------------------------
...
foreach ($m in $matches) {
    ...
    } catch { Write-Host "⚠️ Failed to parse $url" }
}
...
Write-Host "🎯 FULL FK + Data setup complete!"
